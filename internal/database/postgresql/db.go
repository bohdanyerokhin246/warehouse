...
func Connect() {
	connStr := fmt.Sprintf("host=%s port=%s user=%s password=%s dbname=%s sslmode=%s", DBConn.Host, DBConn.Port, DBConn.User, DBConn.Password, DBConn.DbName, DBConn.SSLMode)
	var err error
